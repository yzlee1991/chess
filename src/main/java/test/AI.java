    ...
    private int evaluate(int x, int y, char whoseTurn) {  
        ...
        for (int i = 1; i <= 8; i++) {   
            ...
            if (getLine(x, y, i, -1) == whoseTurn   
                    ...
                    && getLine(x, y, i, -5) == ' '){  
                value += 300000;  
                if((Test.AIToken==false&&whoseTurn=='W')||(Test.AIToken==true&&whoseTurn=='B'))  
                    value+=100000;  
            ...
            if (getLine(x, y, i, -1) == ' '   
                ...
                if(getLine(x, y, i, 1)==' '&&getLine(x, y, i, -5)==' '){  
                    if(Test.whoseTurn==whoseTurn)//可以理解为进攻方  
                        value+=28000;  
                    else//否则防守方的估值不高  
                        value+=2850;  
                }  
                else{  
                    if(Test.whoseTurn==whoseTurn)  
                        value+=2800;  
                    else  
                        value+=1400;  
                }  
...
/*          // 活二的个数 
    ...
    private char getLine(int x, int y, int i, int j) { // i:方向 j:相对x,y的顺序值  
        ...
        return Test.board[x][y].token;  
    }  
  
    // X Y为人下的子，电脑根据xy得到要下的子  
    public void search(int x, int y) {  
        if (Test.step == 1) {// 规定电脑落的第一颗子 跳过搜索 提高效率  
            if (x >= 0 && x <= 7 && y >= 7 && y <= 14)  
                Test.board[x + 1][y + 1].setToken(Test.whoseTurn, x + 1, y + 1, true);  
            else  
                Test.board[x - 1][y - 1].setToken(Test.whoseTurn, x - 1, y - 1, true);  
            Test.whoseTurn = (Test.whoseTurn == 'B') ? 'W' : 'B'; // 改变落子者  
        } else {  
            int val = 0;  
            Alpha_Beta(0, -10000000, 10000000, x, y, Test.whoseTurn);  
            for (int i = 0; i < 14; i++) {  
                for (int j = 0; j < 14; j++) {  
                    if ((val < temp[i][j]) && Test.board[i][j].token == ' ') {  
                        val = temp[i][j];  
                        AIx = i;  
                        AIy = j;  
                    }  
            ...
            Test.board[AIx][AIy].setToken(Test.whoseTurn, AIx, AIy, true);  
            Test.textArea.insertText(0, "估值:" + Test.robot.Evaluate(AIx, AIy, Test.whoseTurn) + " ");  
  
            if (Test.board[AIx][AIy].judge(Test.whoseTurn, AIx, AIy) == true)  
                Test.board[AIx][AIy].printWinner();  
            else  
                Test.whoseTurn = (Test.whoseTurn == 'B') ? 'W' : 'B';  
        }  
    }  
    ...
    private int Alpha_Beta(int depth, int alpha, int beta, int x, int y, char whoseTurn) {  
        ...
        if (depth == 3 || Test.board[x][y].judge(whoseTurn, x, y) == true) {  
            temp[x][y] = Evaluate(x, y, whoseTurn);  
            return Evaluate(x, y, whoseTurn);  
        }  
        for (int i = 0; i < 14; i++) {  
            for (int j = 0; j < 14; j++) {  
                if (Test.board[i][j].token != ' ')  
                    continue;  
                Test.board[i][j].setToken(whoseTurn, i, j, false);  
    ...
    private void unMove(int i, int j) {  
        Test.board[i][j].getChildren().remove(Test.arrayCircle[i][j]);  
        Test.board[i][j].token = ' ';  
        Test.step--;  
    }  
}  