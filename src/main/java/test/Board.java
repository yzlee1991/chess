...
//    private MyCircle circle = null;  
    ...
    public void handleMouseClick(int x, int y) {  
        ...
        if (token == ' ' && Test.whoseTurn != ' ' && Test.AIPlay == false) {  
            // 此处为当人下了一子后 若AIPlay为true 电脑开始下棋 即人机对战 否则为人人对战  
            setToken(Test.whoseTurn, x, y,true);  
            Test.textArea.insertText(0, "估值:" + Test.robot.Evaluate(x, y, Test.whoseTurn) + " ");  
              
            if (judge(Test.whoseTurn, x, y) == true)  
                printWinner();  
            else  
                Test.whoseTurn = (Test.whoseTurn == 'B') ? 'W' : 'B'; // 改变落子者  
...
//            mediaPlayer.play();  
        ...
        //若AIPlay为True 则为人机对战  
        if (token == ' ' && Test.whoseTurn != ' ' && Test.AIPlay == true) {  
            if (Test.whoseTurn != ' ') {  
                setToken(Test.whoseTurn, x, y,true);  
                Test.textArea.insertText(0, "估值:" + Test.robot.Evaluate(x, y, Test.whoseTurn) + " ");  
                if (judge(Test.whoseTurn, x, y) == true)  
...
//                mediaPlayer.play();  
    ...
    public void setToken(char c, int x, int y,boolean flag) {  
        ...
        Test.step++;  
        Test.board[x][y].thisStep = Test.step;// 落一子步数+1 thisStep用于判断悔棋的操作  
        if (token == 'B') {  
            circle = new MyCircle(20, Color.BLACK);  
            // 在textArea上输出相应的落子位置 具体效果为——步数:1 黑棋: 行 3, 列 10  
            if(flag==true)  
            Test.textArea.insertText(0, "步数:" + Test.step + " 黑棋:行" + column + ",列" + row + '\n' + '\n');  
        } else if (token == 'W') {  
            circle = new MyCircle(20, Color.WHITE);  
            if(flag==true)  
            Test.textArea.insertText(0, "步数:" + Test.step + " 白棋:行" + column + ",列" + row + '\n' + '\n');  
        }  
        // 将圆心绑定到落子的方格中间  
        circle.centerXProperty().bind(Test.board[x][y].widthProperty().divide(2));  
        circle.centerYProperty().bind(Test.board[x][y].heightProperty().divide(2));  
        Test.board[x][y].getChildren().add(circle);  
        Test.board[x][y].token = c;  
        Test.arrayCircle[x][y] = circle;  
    }  
    ...
    public int checkCount(char whoseTurn, int x, int y, int xChange, int yChange) {  
        ...
        while (x + xChange >= 0 && x + xChange < 14 && y + yChange >= 0 && y + yChange < 14  
                && whoseTurn == Test.board[x + xChange][y + yChange].token) {  
            count++;  
            if (xChange != 0)  
                xChange++;  
            if (yChange != 0) {  
...
//        mediaPlayer.play();  
        ...
        if (Test.whoseTurn == 'B') {  
            Test.whoseTurn = ' ';  
            Test.textArea.insertText(0, "黑棋获胜！点击开始游戏进行新对战！" + '\n' + '\n');  
            winner.setContentText("黑棋获胜！！！");  
            winner.setHeaderText(null);  
            winner.showAndWait();  
        } else if (Test.whoseTurn == 'W') {  
            Test.whoseTurn = ' ';  
            Test.textArea.insertText(0, "白棋获胜！点击开始游戏进行新对战！" + '\n' + '\n');  
